// @publicdomainrelay/hono-factory-atproto-repo — isomorphic byte helpers
//
// No Node built-ins. Works in Deno + browser via Web APIs.

import type { Bytes } from "../contracts.ts";
...
// ── base32 (RFC 4648, lowercase, no padding — atproto style) ─────
...
export function base32Encode(b: Bytes): string {
  ...
  for (let i = 0; i < b.length; i++) {
    ...
    bits += 8;
    while (bits >= 5) {
      s += BASE32_ALPHABET[(value >>> (bits - 5)) & 0x1f];
      bits -= 5;
    }
  }
  if (bits > 0) {
    s += BASE32_ALPHABET[(value << (5 - bits)) & 0x1f];
  }
  return s;
}
...
export function base32Decode(s: string): Bytes {
  ...
  if (bits > 0 && (value & ((1 << bits) - 1)) !== 0) {
    // trailing bits must be zero (no padding in atproto base32)
    // but we tolerate non-zero trailing bits in practice
  }
