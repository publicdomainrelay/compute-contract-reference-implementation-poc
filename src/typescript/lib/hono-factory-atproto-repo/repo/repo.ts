// @publicdomainrelay/hono-factory-atproto-repo — Repo operations
//
// Wraps the MST to implement the high-level repo API: applyWrites,
// commit signing, and head management.
//
// Commit CBOR structure (dag-cbor):
//   { did, version: 3, data: CID, rev: TID, prev: CID | null, sig: Bytes }
...
import { encode as cborEncode, decode as cborDecode, cidLink } from "../cbor/dag-cbor.ts";
import { cidFromDigest, cidDigest } from "../util/cid.ts";
import { bytesEqual } from "../util/bytes.ts";
import { nextTid } from "../util/tid.ts";
import { createMst, diff, type BlockStore } from "../mst/mst.ts";
...
export interface RepoStorage extends BlockStore, RepoStore {}
...
/** Build the commit object as a plain map, with CID links for data/prev. */
...
/** Extract the MST root CID from a signed commit block. */
async function getMstRoot(store: BlockStore, commitCid: Cid): Promise<Cid | null> {
...
export class Repo implements RepoApi {
  ...
  constructor(store: RepoStorage, signer: Signer, did?: Did) {
    this.#store = store;
    this.#signer = signer;
  ...
  async applyWrites(_did: Did, writes: WriteOp[]): Promise<CommitEvent> {
    ...
    // Extract MST root from previous commit (if any)
    ...
    for (const write of writes) {
      ...
      switch (write.action) {
        ...
        case "update": {
          if (!write.record) {
            throw new XrpcError("InvalidRequest", `record is required for ${write.action}`);
          }
          // Store record as CBOR block
    ...
    // Build an empty MST node if the tree is now empty (all records deleted)
    let rootForCommit = newRoot;
    if (rootForCommit === null) {
      // Create and persist an empty node so the commit has a valid data CID
      const { encode: cborEncode } = await import("../cbor/dag-cbor.ts");
      const emptyBytes = cborEncode({ e: [] });
      const digest = await crypto.subtle.digest("SHA-256", emptyBytes.buffer.slice(emptyBytes.byteOffset, emptyBytes.byteOffset + emptyBytes.byteLength) as ArrayBuffer);
      const { cidFromDigest: cidFromD } = await import("../util/cid.ts");
      rootForCommit = cidFromD(new Uint8Array(digest));
      await this.#store.put(rootForCommit, emptyBytes);
    }

    // Compute diff blocks for the CAR slice
    ...
    const commitData: CommitData = {
      ...
      rev,
      prev: prevCommit,
      sig: new Uint8Array(0), // placeholder
    };

    // Sign the commit (same CBOR repr as stored, minus sig)
    const { sig: _sig, ...dataForSigning } = commitData;
    const bytesToSign = cborEncode(commitToObj(dataForSigning));
    const sig = await this.#signer.sign(bytesToSign);
    commitData.sig = sig;

    // Store the signed commit
...
async function buildCarSlice(store: BlockStore, cids: Cid[]): Promise<Bytes> {
  // Build a minimal CARv1 with just the named blocks
  const parts: Bytes[] = [];

  // CARv1 header: { roots: [], version: 1 }
  const header = cborEncode({ roots: [], version: 1 });
  // varint length prefix for the header
  ...
  for (const cid of cids) {
    ...
    if (!block) continue;
    // Each block: [CID bytes, data bytes]
...
function cidToRawBytes(cid: Cid): Bytes {
  // Strip multibase prefix, decode base32
