// SSRF guard for the one outbound request the x402 client makes.
//
// The payment URL comes from a winning bid's bids.x402 payload — i.e. from a
// counterparty, so it is untrusted input. Before the buyer GETs it we reject
// non-http(s) schemes and the cloud-metadata host, and (opt-in) private/loopback
// ranges. Kept runtime-agnostic: the caller decides whether to block private
// ranges rather than this library reading an env var, so it behaves identically
// on Deno and Node. The reference spindle passes
