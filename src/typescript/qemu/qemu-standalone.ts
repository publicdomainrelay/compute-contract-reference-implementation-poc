...
`;
  ...
  const qemuArgs = [
    ...
    kernelPath,
    "-drive",
    `file=${LIVEOS_IMG},format=raw,if=virtio,readonly=on,cache=none`,
    "-drive",
    `file=${overlayImg},format=raw,if=virtio,cache=none`,
    "-drive",
    `file=${dockerImg},format=raw,if=virtio,cache=none`,
    "-drive",
    `file=${containerdImg},format=raw,if=virtio,cache=none`,
    "-append",
    cfg.kernelAppend,
  ];
